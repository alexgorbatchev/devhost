...
- Create a Go module rooted at `apps/devhost`.
...
- Keep this note file aligned only with items proven by the Go code and tests.

## Validated Evidence

- `go test ./...` passes for the current Go module.
- `go vet ./...` passes for the current Go module.
- This note file now records only the rewrite parity items proven by code plus tests.
...
- A devtools-free `internal/services` stack runner now ports the smallest honest `startStack.ts` path: injected service env, child stdout/stderr prefixing, startup retries, fixed-port and host claims, plain route activation, service URL logging, first-child-exit waiting, and cleanup.
- The new stack-runner tests prove startup-failure cleanup, routed-service cleanup on exit, fixed-port and host claims being present before child spawn, manifest-scoped log labels, routed and non-routed service URL logging, child log prefixing, graceful SIGTERM shutdown, and SIGKILL escalation after the configured grace period.
