...
const SOURCE_MAP_ANNOTATION_PREFIX: string = "sourceMappingURL=";
...
export async function symbolicateSourceLocation(
  ...
  if (
    originalMapping.source === null ||
    originalMapping.line === null ||
    originalMapping.column === null
  ) {
    return undefined;
  }
...
function extractSourceMapUrl(sourceText: string, sourceUrl: string): string | undefined {
  ...
  for (let lineIndex: number = sourceLines.length - 1; lineIndex >= 0; lineIndex -= 1) {
    ...
    const sourceMappingUrlIndex: number = sourceLine.indexOf(SOURCE_MAP_ANNOTATION_PREFIX);

    if (sourceMappingUrlIndex === -1) {
      continue;
    }

    const sourceMapSpecifier: string = sourceLine.slice(
      sourceMappingUrlIndex + SOURCE_MAP_ANNOTATION_PREFIX.length,
    );
