...
- Layering uses the `--devhost-z-*` tokens in `shared/devtools.css`, bottom to top: `overlay` (page highlights), `dock` (toolbar), `popover`, `modal` (terminal windows), `edge` (the minimap, which stays usable above terminals). The Go bundle's CSS minifier (Bun) rounds every number to six significant digits, so keep z-index and other large numeric token values within six significant digits or neighbouring layers collapse into one and stacking falls back to DOM order. Storybook compiles CSS through Vite and does not show this; `TestBundledDevtoolsLayersStackInOrder` in `apps/devhost/internal/devtools/assets_test.go` checks the shipped bundle.
